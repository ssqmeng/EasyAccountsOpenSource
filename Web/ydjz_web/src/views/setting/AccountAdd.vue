<template>
  ...
  <van-divider v-show="this.$route.query.accountId != null && flows.length > 0"
  ...
  <van-list v-show="this.$route.query.accountId != null" v-model="loading" :finished="finished" :finished-text="flows.length > 0 ? '没有更多了' : ''"  v-model:error="error" error-text="请求失败，点击重新加载"
      @load="onLoad">
...
import {showConfirmDialog, closeToast, showDialog, showFailToast, showLoadingToast, showToast} from "vant";
...
export default {
  ...
  handleNumberDelete() {
    ...
    onLoad() {
      if(!this.$route.query.accountId) return;
      if (this.loading) return; // 如果正在加载，直接返回，防止重复调用
      this.loading = true;
      this.$http({
